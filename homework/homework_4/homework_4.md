...
$$
```math
AUC = (1/2) ((0.0 - 0.0) * (0.2 + 0.4) + (0.0 - 0.0) * (0.4 + 0.6) + (0.0 - 0.0) * (0.6 + 0.8) + (0.0 - 0.0) * (0.8 + 1.0) + (0.2 - 0.0) * (1.0 + 1.0) + (0.4 - 0.2) * (1.0 + 1.0) + (0.6 - 0.4) * (1.0 + 1.0) + (0.8 - 0.6) * (1.0 + 1.0) + (1.0 - 0.8) * (1.0 + 1.0))
```

$$
\text{AUC} = \frac{1}{2} \times 2 = 1.0
$$
...
The closest data points to the decision boundary are calledsupport vectors. The goal is to maximize the distance between support vectors to the decision boundary, or equivalently the distance between two lines spanned by support vectors in each of the two classes, which is called `margin`.
...
**(5pt) Exercise:** Suppose we have a decision boundaryX 1 +X 2 −1 = 0 for a binary classification task, and a data point withx 1 = 1 andx 2 = 1, which class should we classify it into, positive or negative? Why?
...
Excellent Fair
```
 Q: Which attribute is better for the classification task? Figure 4.1: Predictor Selection for Splitting
...
```
```
Table 4.2: Help table for weighted average classification error calculation for CreditRating
```
...
72
Figure 4.2: A Regression Tree forSalaryPrediction Based onHitsandYears.
...
**(10pt) Exercise:** For a fully connected multi-layer feed-forward neural network with 1 input layer (units), 2 hidden layers (4 units and 4 unites), and one output layer (3 units), how many parameters are there in total?
...
w 13 w 14 w 23 w 24 w 35 w 45 b 3 b 4 b 5
-0.3 0.2 0.4 -0.1 -0.2 -0.3 0.2 -0.4 0.
